...
const char* lldb_prep_no_cmds = "";/Users/ian/ios-deploy-master/demo.c
...
const CFStringRef get_device_hardware_name(const AMDeviceRef device) {
    ...
    if (kCFCompareEqualTo  == CFStringCompare(model,CFSTR("N56AP"), kCFCompareNonliteral))
        return CFSTR("iPhone 6 (GSM)");
    ...
    if (kCFCompareEqualTo  == CFStringCompare(model,CFSTR("J71AP"), kCFCompareNonliteral))
        return CFSTR("iPad Air (WiFi)");
    if (kCFCompareEqualTo  == CFStringCompare(model,CFSTR("J81AP"), kCFCompareNonliteral))
        return CFSTR("iPad Air 2 (WiFi)");
    if (kCFCompareEqualTo  == CFStringCompare(model,CFSTR("J82AP"), kCFCompareNonliteral))
        return CFSTR("iPad Air 2 (Cellular)");
    if (kCFCompareEqualTo  == CFStringCompare(model,CFSTR("J85MAP"), kCFCompareNonliteral))
        return CFSTR("iPad Mini 3 (WiFi)");
    return model;
...
CFStringRef get_device_full_name(const AMDeviceRef device) {
    ...
    // IMP: these asserts prevent Segmentation fault: 11
...
void handle_device(AMDeviceRef device) {
    ...
    CFStringRef found_device_id = AMDeviceCopyDeviceIdentifier(device),
                device_full_name = get_device_full_name(device), // IMP: Crashed in here until added asserts
                device_interface_name = get_device_interface_name(device);
...
;
    ...
    if (uninstall) {
        ...
        if (bundle_id == NULL)
        {
            path = CFStringCreateWithCString(NULL, app_path, kCFStringEncodingASCII);
            relative_url = CFURLCreateWithFileSystemPath(NULL, path, kCFURLPOSIXPathStyle, false);
            url = CFURLCopyAbsoluteURL(relative_url);
            CFRelease(relative_url);

            cf_bundle_id = get_bundle_id(url);
            if (cf_bundle_id == NULL) {
                printf("Error: Unable to get bundle id from package\n Uninstall failed\n");
            }
        }
        else
        {
            cf_bundle_id = CFStringCreateWithCString(NULL, bundle_id, kCFStringEncodingASCII);
        }
        ...
        if (cf_bundle_id != NULL)
        {
            ...
            int code = AMDeviceSecureUninstallApplication(0, device, cf_bundle_id, 0, NULL, 0);
            if (code == 0) {
                printf("[ OK ] Uninstalled package with bundle id %s\n", MYCFStringCopyUTF8String(cf_bundle_id));
            } else {
...
int main(int argc, char *argv[]) {
    ...
    if (!app_path && !detect_only && !command_only && !uninstall) {
        usage(argv[0]);
        exit(exitcode_error);
    }
